      ...
      <div className="dark:bg-dark">
        <div className="max-w-[800px] mx-auto px-5 lg:py-24 py-12">
          ...
          <Accordion
            type="single"
            collapsible
            className=" shadow-none dark:shadow-none divide-y-1 divide-b-0 divided:border-ld !rounded-none flex flex-col gap-4"
          >
            {Questions.map((item, index) => {
              return (
                <AccordionItem key={index} value={item.key}>
                  <AccordionTrigger className=" focus:ring-0 px-6  text-lg text-ld py-5 border border-ld rounded-md !border-b-none">
                    {item.question}
                  </AccordionTrigger>
                  <AccordionContent className="px-6 py-5 text-base text-ld opacity-80 leading-7 rounded-b-md border border-ld">
                    {item.answer}
                  </AccordionContent>
                </AccordionItem>
              );
