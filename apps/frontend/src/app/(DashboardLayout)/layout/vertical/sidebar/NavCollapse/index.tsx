      ...
      <Collapsible open={open} onOpenChange={setOpen} className="flex flex-col">
        <CollapsibleTrigger asChild aria-controls={`nav-collapse-${item.name}`}>
          <button
            type="button"
            className={twMerge(
              "!text-[15px] flex w-full items-center justify-between gap-3 rounded-full px-4 py-2.5  text-start truncate cursor-pointer  !leading-normal text-link dark:text-white font-normal collapse-menu hover:transform hover:translate-x-1 transition-all duration-200 ease-in-out",
